...
export default function RootLayout({
  ...
  return (
    <html
      lang={locale}
      className={`${inter.variable} ${inter.className} min-h-[100dvh] overscroll-none bg-slate-200 dark:bg-[#050a07]`}
      suppressHydrationWarning
    >
      <body className="flex min-h-[100dvh] flex-col overscroll-none bg-slate-200 text-xactscore-text transition-colors duration-300 dark:bg-[#050a07]">
        ...
        <Script id="theme-init" strategy="beforeInteractive">
          {`(function(){try{var stored=localStorage.getItem("xactscore-theme");var theme=stored||"dark";if(theme==="system"){theme=window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"}var dark=theme==="dark";if(dark){document.documentElement.classList.add("dark")}else{document.documentElement.classList.remove("dark")}var color=dark?"#050a07":"#E2E8F0";document.documentElement.style.backgroundColor=color;var metas=document.querySelectorAll('meta[name="theme-color"]');if(!metas.length){var m=document.createElement("meta");m.setAttribute("name","theme-color");m.setAttribute("content",color);document.head.appendChild(m)}else{metas.forEach(function(meta){meta.removeAttribute("media");meta.setAttribute("content",color)})}}catch(e){document.documentElement.classList.add("dark")}})();`}
        </Script>
        <ThemeProvider>
